...
if len(command_line_args) == 2:
    print ("auto_driver.py <mod_base> <mod_fact>")
    quit()

if len(command_line_args) > 2:
    print(f"Running every {command_line_args[1]}th config")
    mod_base = int(command_line_args[1])
    mod_fact = int(command_line_args[2])
